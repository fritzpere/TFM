...
def feature_vector_per_band(avg_life, std_life, entropy, pooling, avg_midlife, std_midlife):
    ...
    for i_band in range(-1,3):
        ...
        for i_state in range(3):
            trials=len(avg_life[i_band][i_state])
            ...
            for k in range(trials):
                feat_vect[band_dic[i_band]]['dim0'][k]=np.concatenate((np.array([avg_life[0][i_band][i_state][k],std_life[0][i_band][i_state][k],avg_midlife[0][i_band][i_state][k],std_midlife[0][i_band][i_state][k],entropy[0][i_band][i_state][k]]),pooling),axis=0)
                feat_vect[band_dic[i_band]]['dim1'][k]=np.concatenate((np.array([avg_life[1][i_band][i_state][k],std_life[1][i_band][i_state][k],avg_midlife[1][i_band][i_state][k],std_midlife[1][i_band][i_state][k],entropy[1][i_band][i_state][k]]),pooling),axis=0)
