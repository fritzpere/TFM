...
if __name__ == "__main__":
    ...
    for subject in subjects:
        ...
        if debug:
            ts_band=np.concatenate((ts_band[:20,:],ts_band[432:452,:],ts_band[-20:,:]),axis=0)
            labels=np.concatenate((np.zeros(20),np.ones(20),np.ones(20)*2))
                ...
                for i_measure in range(n_measure):
                    for i_vector in range(n_vectors):
                        for i_dim in range(n_dim):
                            ...
                            axes[i_band][i].set_ylabel('accuracy_'+band_dic[i_band]+'_'+str(i_vector),fontsize=8)
                            axes[i_band][i].set_title(band_dic[i_band]+'_'+measures[i_measure]+dimensions[i_dim]+str(i_vector))
                            i=1+i
                ...
                for i_measure in range(n_measure):
                    for i_vector in range(n_vectors):
                        for i_dim in range(n_dim):
                            ...
                            axes2[i_band][i].set_title(band_dic[i_band]+'_'+measures[i_measure]+dimensions[i_dim]+str(i_vector))
                            i=1+i
            fig.tight_layout(pad=0.5)
        ...
        if last:
            ...
            for i_band in bands:
                ...
                #axes[i_band][i].set_xticks([0,1,2,3],['MLR','1NN','control1','control2'])##Provar
                axes[i_band].set_ylabel('accuracy_'+band,fontsize=8)
            ...
            for i_band in bands:
                ...
                for i_clf in range (3):
                    ...
                    axes2[i_band][i_clf].set_title(band+'_'+str(i_clf))
