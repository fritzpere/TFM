        ...
        for sp in range(2):
            ...
            for i_band in bands:
                ...
                for ses in sessions:
                    ...
                    subject_table[table_i,10]=min((labels==0).sum(),(labels==1).sum(),(labels==2).sum())*0.2
                    '''
            ...
            for b in bands:
                band=band_dic[b]
                max_acc[0,b]=np.load('results/intensities/subject_'+str(subject)+'/'+space+'/'+band+'/session'+str(1)+'/topological_clf/'+band+'perf_intensity.npy')[0,1,:].mean()
                max_acc[1,b]=np.load('results/intensities/subject_'+str(subject)+'/'+space+'/'+band+'/session'+str(2)+'/topological_clf/'+band+'perf_intensity.npy')[0,1,:].mean()
